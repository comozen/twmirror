  ...
  return (
    <ul role="list" className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {people.map((person) => (
        <li
          key={person.email}
          className="col-span-1 divide-y divide-white/10 rounded-lg bg-gray-800/50 outline outline-1 -outline-offset-1 outline-white/10"
        >
          <div className="flex w-full items-center justify-between space-x-6 p-6">
            <div className="flex-1 truncate">
              <div className="flex items-center space-x-3">
                <h3 className="truncate text-sm font-medium text-white">{person.name}</h3>
                <span className="inline-flex shrink-0 items-center rounded-full bg-green-500/10 px-1.5 py-0.5 text-xs font-medium text-green-500 ring-1 ring-inset ring-green-500/10">
                  {person.role}
                </span>
              </div>
              <p className="mt-1 truncate text-sm text-gray-400">{person.title}</p>
            </div>
            <img
              alt=""
              src={person.imageUrl}
              className="size-10 shrink-0 rounded-full bg-gray-700 outline outline-1 -outline-offset-1 outline-white/10"
            />
          </div>
          <div>
