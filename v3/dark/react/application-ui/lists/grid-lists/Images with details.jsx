    ...
    <ul role="list" className="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-4 xl:gap-x-8">
      {files.map((file) => (
        <li key={file.source} className="relative">
          <div className="group overflow-hidden rounded-lg bg-gray-800 focus-within:outline focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-indigo-500">
            <img
              alt=""
              src={file.source}
              className="pointer-events-none aspect-[10/7] rounded-lg object-cover outline outline-1 -outline-offset-1 outline-white/10 group-hover:opacity-75"
            />
          ...
          <p className="pointer-events-none block text-sm font-medium text-gray-400">{file.size}</p>
        </li>
      ))}
    </ul>
  )
