  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 z-10 w-screen overflow-y-auto p-4 sm:p-6 md:p-20">
        <DialogPanel
          transition
          className="mx-auto max-w-xl transform divide-y divide-white/10 overflow-hidden rounded-xl bg-gray-900 shadow-2xl outline outline-1 -outline-offset-1 outline-white/10 transition-all data-[closed]:scale-95 data-[closed]:opacity-0 data-[enter]:duration-300 data-[leave]:duration-200 data-[enter]:ease-out data-[leave]:ease-in"
        >
          <Combobox
            onChange={(item) => {
              if (item) {
                    ...
                    <ul className="-mx-4 mt-2 text-sm text-gray-300">
                      {filteredProjects.map((project) => (
                        <ComboboxOption
                          ...
                          key={project.id}
                          value={project}
                          className="group flex cursor-default select-none items-center px-4 py-2 data-[focus]:bg-indigo-500 data-[focus]:text-white data-[focus]:outline-none"
                        >
                          <FolderIcon
                    ...
                    <ul className="-mx-4 mt-2 text-sm text-gray-300">
                      {filteredUsers.map((user) => (
                        <ComboboxOption
                          ...
                          key={user.id}
                          value={user}
                          className="flex cursor-default select-none items-center px-4 py-2 data-[focus]:bg-indigo-500 data-[focus]:text-white"
                        >
                          <img
                            src={user.imageUrl}
                            alt=""
            ...
            <div className="flex flex-wrap items-center bg-gray-800/50 px-4 py-2.5 text-xs text-gray-300">
              ...
              <kbd
                className={classNames(
                  'mx-1 flex size-5 items-center justify-center rounded border bg-gray-800 font-semibold sm:mx-2',
