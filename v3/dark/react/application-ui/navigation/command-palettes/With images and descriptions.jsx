          ...
          >
              ...
              <ComboboxOptions static className="max-h-96 transform-gpu scroll-py-3 overflow-y-auto p-3">
                {filteredItems.map((item) => (
                  <ComboboxOption
                    key={item.id}
                    value={item}
                    className="group flex cursor-default select-none rounded-xl p-3 data-[focus]:bg-white/5 data-[focus]:outline-none"
                  >
                    <div
            ...
            {query !== '' && filteredItems.length === 0 && (
              <div className="px-6 py-14 text-center text-sm sm:px-14">
                <ExclamationCircleIcon
                  type="outline outline-1"
                  name="exclamation-circle"
                  className="mx-auto size-6 text-gray-500"
                />
                <p className="mt-4 font-semibold text-white">No results found</p>
                <p className="mt-2 text-gray-400">No components found for this search term. Please try again.</p>
              </div>
            )}
          </Combobox>
        </DialogPanel>
