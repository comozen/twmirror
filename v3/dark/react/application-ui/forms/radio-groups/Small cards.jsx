      ...
      <div className="mt-2 grid grid-cols-3 gap-3 sm:grid-cols-6">
        {memoryOptions.map((option) => (
          <label
            ...
            className="group relative flex items-center justify-center rounded-md border border-white/10 bg-gray-800/50 p-3 has-[:checked]:border-indigo-500 has-[:disabled]:border-white/10 has-[:checked]:bg-indigo-500 has-[:disabled]:bg-gray-800 has-[:disabled]:opacity-25 has-[:focus-visible]:outline has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-2 has-[:focus-visible]:outline-indigo-500"
          >
            <input
            ...
            />
            <span className="text-sm font-medium uppercase text-white">{option.name}</span>
          </label>
        ))}
      </div>
    </fieldset>
