...
export default function Example() {
  return (
    <Popover className="relative">
      ...
      >
        <div className="w-screen max-w-md flex-auto overflow-hidden rounded-3xl bg-gray-900 text-sm/6 outline outline-1 -outline-offset-1 outline-white/10">
          <div className="p-4">
            {resources.map((item) => (
          ...
          <div className="bg-gray-800/50 p-8">
            <div className="flex justify-between">
              <h3 className="text-sm/6 font-semibold text-gray-400">Recent posts</h3>
            ...
            <ul role="list" className="mt-6 space-y-6">
              {recentPosts.map((post) => (
                <li key={post.id} className="relative">
                  <time dateTime={post.datetime} className="block text-xs/6 text-gray-400">
                    {post.date}
                  </time>
                  <a href={post.href} className="block truncate text-sm/6 font-semibold text-white">
                    {post.title}
                    <span className="absolute inset-0" />
                  </a>
                </li>
