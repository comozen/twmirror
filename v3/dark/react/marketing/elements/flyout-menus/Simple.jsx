...
export default function Example() {
  return (
    <Popover className="relative">
      ...
      >
        <div className="w-56 shrink rounded-xl bg-gray-800 p-4 text-sm/6 font-semibold text-white outline outline-1 -outline-offset-1 outline-white/10">
          {solutions.map((item) => (
            <a key={item.name} href={item.href} className="block p-2 hover:text-indigo-400">
              {item.name}
            </a>
          ))}
        </div>
