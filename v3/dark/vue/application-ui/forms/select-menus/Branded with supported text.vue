<template>
  <Listbox as="div" v-model="selected">
    ...
    <div class="relative">
      <div class="inline-flex divide-x divide-indigo-600 rounded-md outline-none">
        ...
        <ListboxButton class="inline-flex items-center rounded-l-none rounded-r-md bg-indigo-500 p-2 hover:bg-indigo-400 focus-visible:outline focus-visible:outline-2 focus-visible:outline-indigo-400">
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="" leave-to-class="opacity-0">
        <ListboxOptions class="absolute right-0 z-10 mt-2 w-72 origin-top-right divide-y divide-white/10 overflow-hidden rounded-md bg-gray-800 outline outline-1 -outline-offset-1 outline-white/10">
          <ListboxOption as="template" v-for="option in publishingOptions" :key="option.title" :value="option" v-slot="{ active, selected }">
            <li :class="[active ? 'bg-indigo-500 text-white' : 'text-white', 'cursor-default select-none p-4 text-sm']">
