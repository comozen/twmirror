<template>
  <fieldset aria-label="Server size">
    <div class="space-y-4">
      <label v-for="plan in plans" :key="plan.id" :aria-label="plan.name" :aria-description="`${plan.ram}, ${plan.cpus}, ${plan.disk}, ${plan.price} per month`" class="group relative block rounded-lg border border-white/10 bg-gray-800/50 px-6 py-4 has-[:checked]:outline has-[:focus-visible]:outline has-[:checked]:outline-2 has-[:focus-visible]:outline-[3px] has-[:checked]:-outline-offset-2 has-[:focus-visible]:-outline-offset-1 has-[:checked]:outline-indigo-500 sm:flex sm:justify-between">
        <input type="radio" name="plan" :value="plan.id" :checked="plan.id === 'hobby'" class="absolute inset-0 appearance-none focus:outline focus:outline-0" />
        ...
        <span class="mt-2 flex text-sm sm:ml-4 sm:mt-0 sm:flex-col sm:text-right">
          <span class="font-medium text-white">{{ plan.price }}</span>
          <span class="ml-1 text-gray-400 sm:ml-0">/mo</span>
        </span>
      </label>
    </div>
  </fieldset>
