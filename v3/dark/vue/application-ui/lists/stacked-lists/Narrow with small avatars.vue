  ...
  <ul role="list" class="divide-y divide-white/5">
    <li v-for="item in activityItems" :key="item.commit" class="py-4">
      ...
      <p class="mt-3 truncate text-sm text-gray-500">
        Pushed to <span class="text-gray-300">{{ item.projectName }}</span> (<span class="font-mono text-gray-400">{{ item.commit }}</span> on <span class="text-gray-400">{{ item.branch }}</span
        >)
      </p>
    </li>
  </ul>
