<template>
  <div class="bg-gray-900 py-10">
    ...
    <table class="mt-6 w-full whitespace-nowrap text-left">
      ...
      <thead class="border-b border-white/15 text-sm/6 text-white">
        <tr>
        ...
        <tr v-for="item in activityItems" :key="item.commit">
          <td class="py-4 pl-4 pr-8 sm:pl-6 lg:pl-8">
            <div class="flex items-center gap-x-4">
              <img :src="item.user.imageUrl" alt="" class="size-8 rounded-full bg-gray-800 outline outline-1 outline-white/10" />
          ...
          <td class="hidden py-4 pl-0 pr-4 sm:table-cell sm:pr-8">
            <div class="flex gap-x-3">
              ...
              <div class="rounded-md bg-gray-700/40 px-2 py-1 text-xs font-medium text-gray-400 outline outline-1 -outline-offset-1 outline-white/10">{{ item.branch }}</div>
            </div>
          </td>
          <td class="py-4 pl-0 pr-4 text-sm/6 sm:pr-8 lg:pr-20">
            <div class="flex items-center justify-end gap-x-2 sm:justify-start">
              ...
              <div v-if="item.status === 'Completed'" class="flex-none rounded-full bg-green-400/10 p-1 text-green-400">
                <div class="size-1.5 rounded-full bg-current" />
              </div>
              <div v-if="item.status === 'Error'" class="flex-none rounded-full bg-rose-400/10 p-1 text-rose-400">
                <div class="size-1.5 rounded-full bg-current" />
              </div>
