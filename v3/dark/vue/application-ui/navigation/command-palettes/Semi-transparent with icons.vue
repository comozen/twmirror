    ...
    <Dialog class="relative z-10" @close="open = false">
      ...
      <div class="fixed inset-0 z-10 w-screen overflow-y-auto p-4 sm:p-6 md:p-20">
        <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 scale-95" enter-to=" scale-100" leave="ease-in duration-200" leave-from=" scale-100" leave-to="opacity-0 scale-95">
          <DialogPanel class="mx-auto max-w-2xl transform divide-y divide-white/10 overflow-hidden rounded-xl bg-gray-900/80 shadow-2xl outline outline-1 -outline-offset-1 outline-white/10 backdrop-blur backdrop-filter transition-all">
            <Combobox @update:modelValue="onSelect">
              <div class="grid grid-cols-1">
                <ComboboxInput class="col-start-1 row-start-1 h-12 w-full bg-transparent pl-11 pr-4 text-base text-white outline-none placeholder:text-gray-400 sm:text-sm" placeholder="Search..." @change="query = $event.target.value" @blur="query = ''" />
              ...
              <ComboboxOptions v-if="query === '' || filteredProjects.length > 0" static as="ul" class="max-h-80 scroll-py-2 divide-y divide-white/5 overflow-y-auto">
                <li class="p-2">
                  <h2 v-if="query === ''" class="mb-2 mt-4 px-3 text-xs font-semibold text-white">Recent searches</h2>
                  <ul class="text-sm text-gray-300">
                    <ComboboxOption v-for="project in query === '' ? recent : filteredProjects" :key="project.id" :value="project" as="template" v-slot="{ active }">
                      <li :class="['flex cursor-default select-none items-center rounded-md px-3 py-2', active && 'bg-white/5 text-white outline-none']">
                        <FolderIcon :class="['size-6 flex-none', !active && 'text-gray-500', active && 'text-white']" aria-hidden="true" />
                ...
                <li v-if="query === ''" class="p-2">
                  ...
                  <ul class="text-sm text-gray-300">
                    <ComboboxOption v-for="action in quickActions" :key="action.shortcut" :value="action" as="template" v-slot="{ active }">
                      <li :class="['flex cursor-default select-none items-center rounded-md px-3 py-2', active && 'bg-white/5 text-white outline-none']">
                        <component :is="action.icon" :class="['size-6 flex-none', !active && 'text-gray-500', active && 'text-white']" aria-hidden="true" />
              ...
              <div v-if="query !== '' && filteredProjects.length === 0" class="px-6 py-14 text-center sm:px-14">
                ...
                <p class="mt-4 text-sm text-gray-300">We couldn’t find any projects with that term. Please try again.</p>
              </div>
            </Combobox>
          </DialogPanel>
        </TransitionChild>
