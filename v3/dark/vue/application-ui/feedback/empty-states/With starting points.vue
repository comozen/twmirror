<template>
  <div>
    ...
    <p class="mt-1 text-sm text-gray-400">You haven’t created a project yet. Get started by selecting a template or start from an empty project.</p>
    <ul role="list" class="mt-6 grid grid-cols-1 gap-6 border-y border-white/10 py-6 sm:grid-cols-2">
      <li v-for="(item, itemIdx) in items" :key="itemIdx" class="flow-root">
        <div class="relative -m-2 flex items-center space-x-4 rounded-xl p-2 focus-within:outline focus-within:outline-2 focus-within:outline-indigo-500 hover:bg-white/5">
