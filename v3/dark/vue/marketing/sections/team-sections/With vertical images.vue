<template>
  <div class="bg-gray-900 py-24 md:py-32">
    <div class="mx-auto grid max-w-7xl grid-cols-1 gap-20 px-6 lg:px-8 xl:grid-cols-5">
      <div class="max-w-2xl xl:col-span-2">
        <h2 class="text-pretty text-4xl font-semibold tracking-tight text-white sm:text-5xl">About the team</h2>
      ...
      <ul role="list" class="divide-y divide-gray-800 xl:col-span-3">
        <li v-for="person in people" :key="person.name" class="flex flex-col gap-10 py-12 first:pt-0 last:pb-0 sm:flex-row">
          <img class="aspect-[4/5] w-52 flex-none rounded-2xl object-cover outline outline-1 -outline-offset-1 outline-white/10" :src="person.imageUrl" alt="" />
