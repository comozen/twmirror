  ...
  <div class="bg-gray-900 py-24 sm:py-32">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      <div class="mx-auto max-w-2xl lg:max-w-4xl">
        <h2 class="text-pretty text-4xl font-semibold tracking-tight text-white sm:text-5xl">From the blog</h2>
        ...
        <div class="mt-16 space-y-20 lg:mt-20">
          <article v-for="post in posts" :key="post.id" class="relative isolate flex flex-col gap-8 lg:flex-row">
            <div class="relative aspect-video sm:aspect-[2/1] lg:aspect-square lg:w-64 lg:shrink-0">
              <img :src="post.imageUrl" alt="" class="absolute inset-0 size-full rounded-2xl bg-gray-800 object-cover" />
              <div class="absolute inset-0 rounded-2xl ring-1 ring-inset ring-white/10" />
            </div>
            <div>
              <div class="flex items-center gap-x-4 text-xs">
                <time :datetime="post.datetime" class="text-gray-400">{{ post.date }}</time>
                <a :href="post.category.href" class="relative z-10 rounded-full bg-gray-800/60 px-3 py-1.5 font-medium text-gray-300 hover:bg-gray-800">{{ post.category.title }}</a>
              </div>
              <div class="group relative max-w-xl">
                ...
                <p class="mt-5 text-sm/6 text-gray-400">{{ post.description }}</p>
              </div>
              <div class="mt-6 flex border-t border-white/10 pt-6">
                <div class="relative flex items-center gap-x-4">
                  <img :src="post.author.imageUrl" alt="" class="size-10 rounded-full bg-gray-800" />
                  <div class="text-sm/6">
                    ...
                    <p class="text-gray-400">{{ post.author.role }}</p>
                  </div>
                </div>
              </div>
            </div>
