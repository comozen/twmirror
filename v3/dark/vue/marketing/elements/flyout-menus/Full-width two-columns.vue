  ...
  <Popover class="relative isolate z-50 shadow">
    ...
    <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 -translate-y-1" enter-to-class="translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="translate-y-0" leave-to-class="opacity-0 -translate-y-1">
      <PopoverPanel class="absolute inset-x-0 top-16 bg-gray-900">
        ...
        <div class="absolute inset-0 top-1/2 bg-gray-900 ring-1 ring-white/10" aria-hidden="true" />
        <div class="relative bg-gray-900">
          <div class="mx-auto grid max-w-7xl grid-cols-1 gap-x-8 gap-y-10 px-6 py-10 lg:grid-cols-2 lg:px-8">
            <div class="grid grid-cols-2 gap-x-6 sm:gap-x-8">
              <div>
                <h3 class="text-sm/6 font-medium text-gray-400">Engagement</h3>
                <div class="mt-6 flow-root">
                  <div class="-my-2">
                    <a v-for="item in engagement" :key="item.name" :href="item.href" class="flex gap-x-4 py-2 text-sm/6 font-semibold text-white">
                      <component :is="item.icon" class="size-6 flex-none text-gray-500" aria-hidden="true" />
                      {{ item.name }}
                    </a>
                  </div>
                </div>
              ...
              <div>
                <h3 class="text-sm/6 font-medium text-gray-400">Resources</h3>
                <div class="mt-6 flow-root">
                  <div class="-my-2">
                    <a v-for="item in resources" :key="item.name" :href="item.href" class="flex gap-x-4 py-2 text-sm/6 font-semibold text-white">
                      <component :is="item.icon" class="size-6 flex-none text-gray-500" aria-hidden="true" />
                      {{ item.name }}
                    </a>
                  </div>
            ...
            <div class="grid grid-cols-1 gap-10 sm:gap-8 lg:grid-cols-2">
              ...
              <article v-for="post in recentPosts" :key="post.id" class="relative isolate flex max-w-2xl flex-col gap-x-8 gap-y-6 sm:flex-row sm:items-start lg:flex-col lg:items-stretch">
                <div class="relative flex-none">
                  <img class="aspect-[2/1] w-full rounded-lg bg-gray-800 object-cover sm:aspect-video sm:h-32 lg:h-auto" :src="post.imageUrl" alt="" />
                  <div class="absolute inset-0 rounded-lg ring-1 ring-inset ring-white/10" />
                </div>
                <div>
                  <div class="flex items-center gap-x-4">
                    <time :datetime="post.datetime" class="text-sm/6 text-gray-400">{{ post.date }}</time>
                    <a :href="post.category.href" class="relative z-10 rounded-full bg-gray-800 px-3 py-1.5 text-xs font-medium text-gray-400 hover:bg-gray-700">{{ post.category.title }}</a>
                  </div>
                  <h4 class="mt-2 text-sm/6 font-semibold text-white">
                    <a :href="post.href">
                      <span class="absolute inset-0" />
                      {{ post.title }}
                    </a>
                  </h4>
                  <p class="mt-2 text-sm/6 text-gray-400">{{ post.description }}</p>
                </div>
              </article>
            </div>
          </div>
