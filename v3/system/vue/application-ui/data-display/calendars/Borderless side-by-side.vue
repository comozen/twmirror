    ...
    <div class="md:pr-14">
      ...
      <div class="mt-2 grid grid-cols-7 text-sm">
        <div v-for="(day, dayIdx) in days" :key="day.date" :data-first-line="dayIdx <= 6 ? '' : undefined" class="py-2 [&:not([data-first-line])]:border-t [&:not([data-first-line])]:border-gray-200 dark:[&:not([data-first-line])]:border-white/10">
          <button type="button" :data-is-selected="day.isSelected ? '' : undefined" :data-is-current-month="day.isCurrentMonth ? '' : undefined" :data-is-today="day.isToday ? '' : undefined" class="mx-auto flex size-8 items-center justify-center rounded-full data-[is-selected]:data-[is-today]:bg-indigo-600 data-[is-selected]:font-semibold data-[is-today]:font-semibold data-[is-selected]:text-white dark:data-[is-selected]:data-[is-today]:bg-indigo-500 [&:not([data-is-selected])]:hover:bg-gray-200 [&:not([data-is-selected])]:data-[is-today]:text-indigo-600 dark:[&:not([data-is-selected])]:hover:bg-white/10 dark:[&:not([data-is-selected])]:data-[is-today]:text-indigo-400 data-[is-selected]:[&:not([data-is-today])]:bg-gray-900 dark:data-[is-selected]:[&:not([data-is-today])]:bg-white dark:data-[is-selected]:[&:not([data-is-today])]:text-gray-900 [&:not([data-is-selected])]:[&:not([data-is-today])]:data-[is-current-month]:text-gray-900 dark:[&:not([data-is-selected])]:[&:not([data-is-today])]:data-[is-current-month]:text-white [&:not([data-is-selected])]:[&:not([data-is-today])]:[&:not([data-is-current-month])]:text-gray-400 dark:[&:not([data-is-selected])]:[&:not([data-is-today])]:[&:not([data-is-current-month])]:text-gray-500">
    ...
    <section class="mt-12 md:mt-0 md:pl-14">
      ...
      <ol class="mt-4 flex flex-col gap-y-1 text-sm/6 text-gray-500 dark:text-gray-400">
        <li v-for="meeting in meetings" :key="meeting.id" class="group flex items-center gap-x-4 rounded-xl px-4 py-2 focus-within:bg-gray-100 hover:bg-gray-100 dark:focus-within:bg-white/5 dark:hover:bg-white/5">
          <img :src="meeting.imageUrl" alt="" class="size-10 flex-none rounded-full dark:outline dark:outline-1 dark:-outline-offset-1 dark:outline-white/10" />
          <div class="flex-auto">
          ...
          <Menu as="div" class="relative opacity-0 focus-within:opacity-100 group-hover:opacity-100">
            <MenuButton class="relative flex items-center rounded-full text-gray-500 outline-offset-[6px] hover:text-gray-600 dark:text-gray-400 dark:hover:text-white">
