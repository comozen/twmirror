              ...
              <Disclosure as="div" v-else v-slot="{ open }">
                ...
                <DisclosurePanel as="ul" class="mt-1 px-2">
                  <li v-for="subItem in item.children" :key="subItem.name">
                    <!-- 44px -->
                    <DisclosureButton as="a" :href="subItem.href" :class="[subItem.current ? 'bg-gray-50 dark:bg-white/5 dark:text-white' : 'hover:bg-gray-50 dark:hover:bg-white/5 dark:hover:text-white', 'block rounded-md py-2 pl-9 pr-2 text-sm/6 text-gray-700 dark:text-gray-400']">{{ subItem.name }}</DisclosureButton>
                  </li>
                </DisclosurePanel>
              </Disclosure>
            </li>
