  ...
  <Popover class="relative">
    ...
    <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1" enter-to-class="translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="translate-y-0" leave-to-class="opacity-0 translate-y-1">
      <PopoverPanel class="absolute left-1/2 z-10 mt-5 flex w-screen max-w-max -translate-x-1/2 bg-transparent px-4">
        <div class="w-screen max-w-md flex-auto overflow-hidden rounded-3xl bg-white text-sm/6 shadow-lg outline outline-1 outline-gray-900/5 dark:bg-gray-900 dark:shadow-none dark:-outline-offset-1 dark:outline-white/10">
          <div class="p-4">
            <div v-for="item in resources" :key="item.name" class="group relative flex gap-x-6 rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-white/5">
          ...
          <div class="bg-gray-50 p-8 dark:bg-gray-800/50">
            ...
            <ul role="list" class="mt-6 space-y-6">
              <li v-for="post in recentPosts" :key="post.id" class="relative">
                <time :datetime="post.datetime" class="block text-xs/6 text-gray-600 dark:text-gray-400">{{ post.date }}</time>
