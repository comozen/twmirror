  ...
  <Popover class="relative isolate z-50 shadow">
    ...
    <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 -translate-y-1" enter-to-class="translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="translate-y-0" leave-to-class="opacity-0 -translate-y-1">
      <PopoverPanel class="absolute inset-x-0 top-16 bg-white">
        ...
        <div class="absolute inset-0 top-1/2 bg-white shadow-lg ring-1 ring-gray-900/5" aria-hidden="true" />
            ...
            <div class="grid grid-cols-2 gap-x-6 sm:gap-x-8">
              <div>
                ...
                <div class="mt-6 flow-root">
                  <div class="-my-2">
                    <a v-for="item in engagement" :key="item.name" :href="item.href" class="flex gap-x-4 py-2 text-sm/6 font-semibold text-gray-900">
                      <component :is="item.icon" class="size-6 flex-none text-gray-400" aria-hidden="true" />
                      {{ item.name }}
                    </a>
                  </div>
                </div>
              ...
              <div>
                ...
                <div class="mt-6 flow-root">
                  <div class="-my-2">
                    <a v-for="item in resources" :key="item.name" :href="item.href" class="flex gap-x-4 py-2 text-sm/6 font-semibold text-gray-900">
                      <component :is="item.icon" class="size-6 flex-none text-gray-400" aria-hidden="true" />
                      {{ item.name }}
                    </a>
                  </div>
            ...
            <div class="grid grid-cols-1 gap-10 sm:gap-8 lg:grid-cols-2">
              ...
              <article v-for="post in recentPosts" :key="post.id" class="relative isolate flex max-w-2xl flex-col gap-x-8 gap-y-6 sm:flex-row sm:items-start lg:flex-col lg:items-stretch">
                <div class="relative flex-none">
                  <img class="aspect-[2/1] w-full rounded-lg bg-gray-100 object-cover sm:aspect-video sm:h-32 lg:h-auto" :src="post.imageUrl" alt="" />
                  <div class="absolute inset-0 rounded-lg ring-1 ring-inset ring-gray-900/10" />
                </div>
                <div>
                  <div class="flex items-center gap-x-4">
                    <time :datetime="post.datetime" class="text-sm/6 text-gray-600">{{ post.date }}</time>
                  ...
                  <p class="mt-2 text-sm/6 text-gray-600">{{ post.description }}</p>
                </div>
              </article>
            </div>
          </div>
