<template>
  <div>
    ...
    <div class="mt-2 grid grid-cols-7 text-sm">
      <div v-for="(day, dayIdx) in days" :key="day.date" :data-first-line="dayIdx <= 6 ? '' : undefined" class="py-2 [&:not([data-first-line])]:border-t [&:not([data-first-line])]:border-gray-200">
    ...
    <section class="mt-12">
      ...
      <ol class="mt-4 flex flex-col gap-y-1 text-sm/6 text-gray-500">
        <li v-for="meeting in meetings" :key="meeting.id" class="group flex items-center gap-x-4 rounded-xl px-4 py-2 focus-within:bg-gray-100 hover:bg-gray-100">
          ...
          <Menu as="div" class="relative opacity-0 focus-within:opacity-100 group-hover:opacity-100">
            <MenuButton class="relative flex items-center rounded-full text-gray-500 outline-offset-[6px] hover:text-gray-600">
            ...
            <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform scale-100" leave-to-class="transform opacity-0 scale-95">
              <MenuItems class="absolute right-0 z-10 mt-2 w-36 origin-top-right rounded-md bg-white shadow-lg outline outline-1 outline-black/5">
                <div class="py-1">
