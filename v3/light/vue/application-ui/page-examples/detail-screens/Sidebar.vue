  ...
  <div>
                ...
                <nav class="relative flex flex-1 flex-col">
                  <ul role="list" class="flex flex-1 flex-col gap-y-7">
                    <li>
                      <ul role="list" class="-mx-2 space-y-1">
                        <li v-for="item in navigation" :key="item.name">
                          <a :href="item.href" :class="[item.current ? 'bg-gray-100 text-indigo-600' : 'text-gray-700 hover:bg-gray-100 hover:text-indigo-600', 'group flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold']">
                    ...
                    <li>
                      ...
                      <ul role="list" class="-mx-2 mt-2 space-y-1">
                        <li v-for="team in teams" :key="team.name">
                          <a :href="team.href" :class="[team.current ? 'bg-gray-100 text-indigo-600' : 'text-gray-700 hover:bg-gray-100 hover:text-indigo-600', 'group flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold']">
                    ...
                    <li class="-mx-6 mt-auto">
                      <a href="#" class="flex items-center gap-x-4 px-6 py-3 text-sm/6 font-semibold text-gray-900 hover:bg-gray-100">
                        <img class="size-8 rounded-full bg-gray-100 outline outline-1 -outline-offset-1 outline-black/5" src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80" alt="" />
    ...
    <div class="xl:pl-72">
      ...
      <div class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-6 border-b border-gray-200 bg-white px-4 shadow-sm sm:px-6 lg:px-8">
        <button type="button" class="-m-2.5 p-2.5 text-gray-900 xl:hidden" @click="sidebarOpen = true">
        ...
        <div class="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
          <form class="grid flex-1 grid-cols-1" action="#" method="GET">
            <input name="search" aria-label="Search" class="col-start-1 row-start-1 block size-full bg-transparent pl-8 text-base text-gray-900 outline-none placeholder:text-gray-400 sm:text-sm/6" placeholder="Search" />
      ...
      <main>
          ...
          <nav class="flex overflow-x-auto border-b border-gray-200 py-4">
            <ul role="list" class="flex min-w-full flex-none gap-x-6 px-4 text-sm/6 font-semibold text-gray-500 sm:px-6 lg:px-8">
              <li v-for="item in secondaryNavigation" :key="item.name">
                <a :href="item.href" :class="item.current ? 'text-indigo-600' : ''">{{ item.name }}</a>
              </li>
            </ul>
          </nav>
            ...
            <div>
              <div class="flex items-center gap-x-3">
                ...
                <h1 class="flex gap-x-3 text-base/7">
                  <span class="font-semibold text-gray-900">Planetaria</span>
                  <span class="text-gray-400">/</span>
                  <span class="font-semibold text-gray-900">mobile-api</span>
                </h1>
              </div>
              <p class="mt-2 text-xs/6 text-gray-500">Deploys from GitHub via main branch</p>
            </div>
            <div class="order-first flex-none rounded-full bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-500 ring-1 ring-inset ring-indigo-200 sm:order-none">Production</div>
          </div>

          <!-- Stats -->
          <div class="grid grid-cols-1 bg-gray-50 sm:grid-cols-2 lg:grid-cols-4">
            <div v-for="(stat, statIdx) in stats" :key="stat.name" :class="[statIdx % 2 === 1 ? 'sm:border-l' : statIdx === 2 ? 'lg:border-l' : '', 'border-t border-gray-200/50 px-4 py-6 sm:px-6 lg:px-8']">
        ...
        <div class="border-t border-gray-200 pt-11">
          <h2 class="px-4 text-base/7 font-semibold text-gray-900 sm:px-6 lg:px-8">Latest activity</h2>
              ...
              <tr v-for="item in activityItems" :key="item.commit">
                <td class="py-4 pl-4 pr-8 sm:pl-6 lg:pl-8">
                  <div class="flex items-center gap-x-4">
                    <img :src="item.user.imageUrl" alt="" class="size-8 rounded-full bg-gray-100 outline outline-1 -outline-offset-1 outline-black/5" />
                    <div class="truncate text-sm/6 font-medium text-gray-900">{{ item.user.name }}</div>
                  </div>
                </td>
                <td class="hidden py-4 pl-0 pr-4 sm:table-cell sm:pr-8">
                  <div class="flex gap-x-3">
                    <div class="font-mono text-sm/6 text-gray-500">{{ item.commit }}</div>
                    <span class="inline-flex items-center rounded-md bg-gray-50 px-2 py-1 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-300">{{ item.branch }}</span>
                  </div>
                </td>
                <td class="py-4 pl-0 pr-4 text-sm/6 sm:pr-8 lg:pr-20">
                  <div class="flex items-center justify-end gap-x-2 sm:justify-start">
                    <time class="text-gray-500 sm:hidden" :datetime="item.dateTime">{{ item.date }}</time>
                ...
                <td class="hidden py-4 pl-0 pr-8 text-sm/6 text-gray-500 md:table-cell lg:pr-20">{{ item.duration }}</td>
                <td class="hidden py-4 pl-0 pr-4 text-right text-sm/6 text-gray-500 sm:table-cell sm:pr-6 lg:pr-8">
