  ...
  <Combobox as="div" v-model="selectedPerson" @update:modelValue="query = ''">
    ...
    <div class="relative mt-2">
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="" leave-to-class="opacity-0">
        <ComboboxOptions v-if="filteredPeople.length > 0 || query.length > 0" class="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg outline outline-1 outline-black/5 sm:text-sm">
          <ComboboxOption v-if="queryPerson" :value="queryPerson" as="template" v-slot="{ active }">
            <li :class="['relative cursor-default select-none px-3 py-2', active ? 'bg-indigo-600 text-white outline-none' : 'text-gray-900']">
              <div class="flex items-center">
                <div :class="['grid size-6 shrink-0 place-items-center rounded-full', active ? 'bg-white' : 'bg-gray-300']">
                  <UserIcon :class="['size-4', active ? 'fill-indigo-600' : 'fill-white']" aria-hidden="true" />
                </div>
          ...
          <ComboboxOption v-for="person in filteredPeople" :key="person.id" :value="person" as="template" v-slot="{ active }">
            <li :class="['relative cursor-default select-none px-3 py-2', active ? 'bg-indigo-600 text-white outline-none' : 'text-gray-900']">
              <div class="flex items-center">
                <img :src="person.imageUrl" alt="" class="size-6 shrink-0 rounded-full bg-gray-100 outline outline-1 -outline-offset-1 outline-black/5" />
                <span class="ml-3 truncate">
                  {{ person.name }}
                </span>
              </div>
