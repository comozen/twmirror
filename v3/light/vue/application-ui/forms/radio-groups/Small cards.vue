  ...
  <fieldset aria-label="Choose a memory option">
    ...
    <div class="mt-2 grid grid-cols-3 gap-3 sm:grid-cols-6">
      <label v-for="option in memoryOptions" :key="option.id" :aria-label="option.name" class="group relative flex items-center justify-center rounded-md border border-gray-300 bg-white p-3 has-[:checked]:border-indigo-600 has-[:disabled]:border-gray-400 has-[:checked]:bg-indigo-600 has-[:disabled]:bg-gray-200 has-[:disabled]:opacity-25 has-[:focus-visible]:outline has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-2 has-[:focus-visible]:outline-indigo-600">
        ...
        <span class="text-sm font-medium uppercase text-gray-900 group-has-[:checked]:text-white">{{ option.name }}</span>
      </label>
    </div>
  </fieldset>
</template>
