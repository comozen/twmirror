<template>
  <fieldset>
    ...
    <div class="mt-6 flex items-center gap-x-3">
      <div v-for="color in options" :key="color.id" class="flex rounded-full outline outline-1 -outline-offset-1 outline-black/10">
        <input :aria-label="color.name" type="radio" name="color" :value="color.id" :checked="color === options[0]" :class="[color.classes, 'size-8 appearance-none rounded-full forced-color-adjust-none checked:outline checked:outline-2 checked:outline-offset-2 focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-offset-[3px]']" />
      </div>
    </div>
  </fieldset>
</template>
