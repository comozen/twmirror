<template>
  <ul role="list" class="grid grid-cols-1 gap-x-6 gap-y-8 lg:grid-cols-3 xl:gap-x-8">
    <li v-for="client in clients" :key="client.id" class="overflow-hidden rounded-xl outline outline-gray-200">
      ...
      <dl class="-my-3 divide-y divide-gray-100 px-6 py-4 text-sm/6">
        ...
        <div class="flex justify-between gap-x-4 py-3">
          ...
          <dd class="flex items-start gap-x-2">
            ...
            <div v-if="client.lastInvoice.status == 'Paid'" class="rounded-md bg-green-50 px-2 py-1 text-xs font-medium text-green-700 ring-1 ring-green-600/20 ring-inset">{{ client.lastInvoice.status }}</div>
            <div v-if="client.lastInvoice.status == 'Withdraw'" class="rounded-md bg-gray-50 px-2 py-1 text-xs font-medium text-gray-600 ring-1 ring-gray-500/10 ring-inset">{{ client.lastInvoice.status }}</div>
            <div v-if="client.lastInvoice.status == 'Overdue'" class="rounded-md bg-red-50 px-2 py-1 text-xs font-medium text-red-700 ring-1 ring-red-600/10 ring-inset">{{ client.lastInvoice.status }}</div>
          </dd>
        </div>
      </dl>
    </li>
