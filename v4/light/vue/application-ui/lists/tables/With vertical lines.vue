          ...
          <table class="min-w-full divide-y divide-gray-300">
            <thead>
              <tr class="divide-x divide-gray-200">
                ...
                <th scope="col" class="py-3.5 pr-4 pl-4 text-left text-sm font-semibold text-gray-900 sm:pr-0">Role</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr v-for="person in people" :key="person.email" class="divide-x divide-gray-200">
                <td class="py-4 pr-4 pl-4 text-sm font-medium whitespace-nowrap text-gray-900 sm:pl-0">{{ person.name }}</td>
                <td class="p-4 text-sm whitespace-nowrap text-gray-500">{{ person.title }}</td>
                <td class="p-4 text-sm whitespace-nowrap text-gray-500">{{ person.email }}</td>
                <td class="py-4 pr-4 pl-4 text-sm whitespace-nowrap text-gray-500 sm:pr-0">{{ person.role }}</td>
              </tr>
            </tbody>
          </table>
        </div>
