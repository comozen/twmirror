        ...
        <div class="relative flex items-center justify-center py-5 lg:justify-between">
          ...
          <div class="hidden lg:ml-4 lg:flex lg:items-center lg:pr-0.5">
            ...
            <Menu as="div" class="relative ml-4 shrink-0">
              ...
              <transition leave-active-class="transition ease-in duration-75" leave-from-class="transform scale-100" leave-to-class="transform opacity-0 scale-95">
                <MenuItems class="absolute right-0 z-10 mt-2 -mr-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg outline outline-black/5">
          ...
          <div class="min-w-0 flex-1 px-12 lg:hidden">
            <div class="mx-auto grid w-full max-w-xs grid-cols-1">
              <input name="search" aria-label="Search" class="col-start-1 row-start-1 block w-full rounded-md bg-indigo-500/75 py-1.5 pr-3 pl-10 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-white/75 focus:outline-2 focus:-outline-offset-2 focus:outline-white sm:text-sm/6" placeholder="Search" />
              <MagnifyingGlassIcon class="pointer-events-none col-start-1 row-start-1 ml-3 size-5 self-center text-white/75" aria-hidden="true" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-3 items-center gap-8">
            <div class="col-span-2">
              <nav class="flex space-x-4">
                <a v-for="item in navigation" :key="item.name" :href="item.href" :class="[item.current ? 'text-white' : 'text-indigo-100', 'rounded-md px-3 py-2 text-sm font-medium hover:bg-indigo-500/75']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</a>
              </nav>
            </div>
        ...
        <Dialog class="lg:hidden" @close="open = false">
          ...
          <TransitionChild as="template" enter="duration-150 ease-out" enter-from="opacity-0 scale-95" enter-to=" scale-100" leave="duration-150 ease-in" leave-from=" scale-100" leave-to="opacity-0 scale-95">
            <DialogPanel class="absolute inset-x-0 top-0 z-30 mx-auto w-full max-w-3xl origin-top transform p-2 transition">
              <div class="divide-y divide-gray-200 rounded-lg bg-white shadow-lg outline outline-black/5">
                <div class="pt-3 pb-2">
                  <div class="flex items-center justify-between px-4">
                    ...
                    <div class="-mr-2">
                      <button class="relative inline-flex items-center justify-center rounded-md bg-white p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-white" @click="open = false">
                        <span class="absolute -inset-0.5" />
                ...
                <div class="pt-4 pb-2">
                  <div class="flex items-center px-5">
                    <div class="shrink-0">
                      <img class="size-10 rounded-full outline -outline-offset-1 outline-black/5" :src="user.imageUrl" alt="" />
                    </div>
                    ...
                    <button type="button" class="relative ml-auto shrink-0 rounded-full p-1 text-gray-400 hover:text-gray-500 focus:outline-2 focus:outline-offset-2 focus:outline-indigo-500">
                      <span class="absolute -inset-1.5" />
                      <span class="sr-only">View notifications</span>
                      <BellIcon class="size-6" aria-hidden="true" />
                    </button>
