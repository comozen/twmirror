  ...
  <Combobox as="div" v-model="selectedPerson" @update:modelValue="query = ''">
    ...
    <div class="relative mt-2">
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="" leave-to-class="opacity-0">
        <ComboboxOptions v-if="filteredPeople.length > 0 || query.length > 0" class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg outline-1 outline-black/5 sm:text-sm">
          <ComboboxOption v-if="queryPerson" :value="queryPerson" as="template" v-slot="{ active }">
            <li :class="['relative cursor-default px-3 py-2 select-none', active ? 'bg-indigo-600 text-white outline-hidden' : 'text-gray-900']">
              <div class="flex items-center">
                <span class="inline-block size-2 shrink-0 rounded-full border border-gray-400 in-aria-active:border-white/75" aria-hidden="true" />
